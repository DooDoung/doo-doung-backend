...
function parseValue(value: string, type: 'string' | 'number' | 'boolean' | 'date' | 'datetime' | 'time' | 'decimal'): any {
  ...
  for (const row of data) {
    try {
      ...
      if (usedUsernames.has(username)) {
        console.warn(`⚠️  Skipping account ${row.id}: duplicate username ${username}`)
        errorCount++
        continue
      }
  ...
  for (const row of data) {
    ...
    try {
      ...
      if (!existingAccountIds.has(accountId)) {
        console.warn(`⚠️  Skipping prophet ${accountId}: account not found or not a prophet account`)
        errorCount++
        continue
      }
...
async function seedProphetAvailabilities() {
  ...
  for (const row of data) {
    ...
    try {
      ...
      console.warn(`⚠️  Error seeding prophet availability for prophet ${prophetId}:`, error.message)
      skipCount++
    }
  }
  
  console.log(`✅ Seeded ${successCount} prophet availabilities (${skipCount} skipped)`)
}
...
async function seedTransactions() {
  ...
  for (const row of data) {
    try {
      ...
      if (!bookingExists) {
        console.warn(`⚠️  Skipping transaction ${row.id}: booking ${bookingId} not found`)
        errorCount++
        continue
      }
  ...
  console.log(`✅ Seeded ${successCount} transactions (${errorCount} skipped due to errors)`)
}
...
async function seedReviews() {
  ...
  for (const row of data) {
    try {
      ...
      if (!customerExists) {
        console.warn(`⚠️  Skipping review ${row.id}: customer ${customerId} not found`)
        errorCount++
        continue
      }
      
      if (!bookingExists) {
        console.warn(`⚠️  Skipping review ${row.id}: booking ${bookingId} not found`)
        errorCount++
        continue
      }
  ...
  console.log(`✅ Seeded ${successCount} reviews (${errorCount} skipped due to errors)`)
}
...
async function seedReports() {
  ...
  for (const row of data) {
    try {
      ...
      if (!customerExists) {
        console.warn(`⚠️  Skipping report ${row.id}: customer ${customerId} not found`)
        errorCount++
        continue
      }
      ...
      if (adminId) {
        ...
        if (!adminExists) {
          console.warn(`⚠️  Skipping report ${row.id}: admin ${adminId} not found`)
          errorCount++
          continue
        }
      }
  ...
  console.log(`✅ Seeded ${successCount} reports (${errorCount} skipped due to errors)`)
}
